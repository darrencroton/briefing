...
def parse_noted_config(notes: str | None) -> RecordingConfig | None:
    ...
    for index, line in enumerate(lines):
        ...
        if not isinstance(raw, dict):
            raise PlanningError("noted config marker must contain YAML key/value fields")
...
def recording_config_from_mapping(raw: dict[str, Any]) -> RecordingConfig:
    ...
    if not isinstance(participants, dict):
        raise PlanningError("noted config participants must be a mapping")
    transcription = raw.get("transcription") or {}
    if not isinstance(transcription, dict):
        raise PlanningError("noted config transcription must be a mapping")
    policy_raw = raw.get("recording_policy") or raw.get("policy") or {}
    if not isinstance(policy_raw, dict):
        raise PlanningError("noted config recording_policy must be a mapping")
